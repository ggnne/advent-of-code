...
int getWinCount(std::string line, std::map<int, int> &memory){

    size_t pos = line.find(": ");
    std::string strCardID = line.substr(0, pos);
    strCardID = strCardID.erase(0, strCardID.find(" "));
    int cardID = std::stoi(strCardID);

    if (memory.count(cardID-1)){
        return memory[cardID-1];
    }

    std::string winning, available;
    std::stringstream wss, avss;
    std::set<int> wset;
    ...
    pos = line.find(" | ");
    winning = line.substr(0, pos);
    available = line.erase(0, pos + 3);
...
int main(int argc, char** argv){
    ...
    while (std::getline(fin, line)){
        ...
        int wins = getWinCount(line, memory);
        for (int k = i + 1; k < i + wins + 1; ++k){
            Q.push(k);
        }

        memory.insert({i, wins});
        sum++;
        i++;
    }
    ...
    while (!Q.empty()){
        ...
        line = scratchcards[copy];
        int wins = getWinCount(line, memory);
        for (int k = copy + 1; k < copy + wins + 1; ++k){
            Q.push(k);
        }
        sum++;
    }
